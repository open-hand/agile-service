...
export default function useIssueTypes(config?: IssueTypeConfig, options?: UseQueryOptions<IIssueType[]>) {
  ...
  return useQuery(key, () => (isOrganization ? issueTypeApi.orgLoad({ params: { page: 0, size: 0 }, data: {} }) : issueTypeApi.loadAllWithStateMachineId(config?.applyType ?? applyType, config?.id, config?.onlyEnabled)), {
    select: (res) => {
      let issueTypes = [];
      if (isOrganization) {
        issueTypes = ((Array.isArray(res) ? res : res.list) || []).filter((item: IIssueType) => item.typeCode !== 'backlog');
      } else {
        issueTypes = (!isProgram ? res.filter((item: IIssueType) => item.typeCode !== 'feature') : res);
      }
