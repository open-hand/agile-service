...
const SelectVersion: React.FC<SelectVersionProps> = observer(({
  ...
  const loadData = useCallback(() => {
    ...
    if (programMode) {
      versionId && subProjectId && publishVersionApi.loadProgramAppService(versionId, subProjectId).then((res: any) => {
        setOptions(res);
      });
    } else {
      versionId && serviceCode && publishVersionApi.loadAvailableAppVersionList(versionId, serviceCode).then((res: any) => {
  ...
  function handleCheckVersion(name: string) {
    return publishVersionApi.checkAppVersion({ artifactId: serviceCode!, serviceCode: serviceCode!, version: name });
  }
                      ...
                      <Button
                        ...
                        onClick={async (e) => {
                          ...
                          if (await inputRef.current?.validate(versionName) && serviceCode) {
                            const newVersion: IAppVersionData = await publishVersionApi.createAppVersion({ artifactId: serviceCode!, serviceCode, version: versionName });
