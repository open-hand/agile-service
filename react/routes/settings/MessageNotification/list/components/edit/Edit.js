...
import {
  Checkbox, Select, Tooltip, Modal, Spin,
} from 'choerodon-ui';
...
class Edit extends Component {
  ...
  componentDidMount() {
    const { edit: noticeType } = this.props;
    axios.all([
      axios.get(`/agile/v1/projects/${AppState.currentMenuType.id}/notice`),
      axios.get(`/base/v1/projects/${AppState.currentMenuType.id}/users`),
    ])
        ...
        render: (text, record, index) => (index > 2 ? (
          <Select
            ...
            onFilterChange={(param) => {
              ...
              axios.get(`/base/v1/projects/${AppState.currentMenuType.id}/users?param=${param}`)
                .then((res) => {
                  this.setState({
                    userOptionsLoading: false,
  ...
  renderNotifies = () => {
    ...
    return dataSource.map((data, index) => {
      const { key, typeName } = data;
      return (
        <Fragment>
          <Checkbox
            style={{ marginBottom: 20, marginLeft: 0, display: 'block' }}
            checked={checkeds[index]}
            onChange={e => this.handleCheckboxChange(e, index)}
          >
            {typeName}
  ...
  render() {
    ...
    return (
      <Sidebar title="编辑通知对象" visible okText="保存" onOk={this.handleSaveBtnClick} onCancel={onCancel} width={380}>
