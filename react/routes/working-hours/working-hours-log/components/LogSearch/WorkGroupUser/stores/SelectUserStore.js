...
class IssueTreeStore {
  constructor({ ROOT_ID, NOT_ASSIGN_ID }) {
    this.ROOT_ID = ROOT_ID;
    this.NOT_ASSIGN_ID = NOT_ASSIGN_ID;
  }
  ...
  async loadIssueTree(initCaseSelected) {
    ...
    const treeData = await workGroupApi.loadWorkGroup();
  ...
  @action setTreeData(treeData, initCaseSelected) {
    ...
    if (flattenedTree) {
      expandedKeys = flattenedTree.map((node) => node.item)
  ...
  removeFolderSelectedCase(folderId, caseId) {
    ...
    if (item.mainSelect) {
      ...
      if (item.userCount === item.unSelected.length) {
        this.handleCheckChange(false, folderId);
      }
    }
  }
