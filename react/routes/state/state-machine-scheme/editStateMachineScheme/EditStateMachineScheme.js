...
class EditStateMachineScheme extends Component {
  ...
  renderStateMachine = (value, record) => {
    const text = (record.stateMachineVO && record.stateMachineVO.length !== 0 && (
      <Fragment>{record.stateMachineVO.name}</Fragment>));
    const isHasEditorDel = record.issueTypeVOS && record.issueTypeVOS.length && record.issueTypeVOS[0].id && this.state.showStatus === 'draft';
    const menu = (
      <Menu onClick={this.handleDelete.bind(this, record.stateMachineVO && record.stateMachineVO.id)}>
    ...
    return (
      <TableDropMenu
        text={text}
        className={!isHasEditorDel && `${prefixCls}-table-col-td-edit`}
        onClickEdit={isHasEditorDel && this.handleEditStateMachine.bind(this, record.stateMachineVO && record.stateMachineVO.id)}
        menu={isHasEditorDel && menu}
      />
    );
  };
  ...
  getColumns = () => [
    {
      ...
      //   )
      // ),
      render: this.renderStateMachine,
      filters: [],
      onFilter: (value, record) => record.stateMachineVO.name.toString().indexOf(value) === 0,
    },
    {
  ...
  render() {
    ...
    return (
      <Page className="issue-state-machine-scheme-edit">
        ...
        // backPath={from
        //   ? `/agile/states/machine?type=${type}&id=${projectId}&name=${encodeURIComponent(name)}&organizationId=${orgId}`
        //   : `/agile/states/scheme?type=${type}&id=${projectId}&name=${encodeURIComponent(name)}&organizationId=${orgId}`
        // }
        >
        ...
        <Breadcrumb custom>
          ...
          <Item>
            <Link to={from
              ? `/agile/states/state-machine?type=${type}&id=${projectId}&name=${encodeURIComponent(name)}&organizationId=${orgId}`
              : `/agile/states/scheme?type=${type}&id=${projectId}&name=${encodeURIComponent(name)}&organizationId=${orgId}`
            }
            >
              {
...
export default Form.create({})(injectIntl(EditStateMachineScheme));
