...
import {
  Table, Button, Modal, Form, Icon, Select, Input, Dropdown,
  Tabs, Checkbox, Popconfirm, Spin, Menu,
  Breadcrumb as Bread,
} from 'choerodon-ui';
import { injectIntl, FormattedMessage } from 'react-intl';
import {
  Content, Header, Page, Permission, stores, Breadcrumb, Choerodon
} from '@choerodon/boot';
...
import { statusApi, stateMachineApi } from '@/api';
...
class EditStateMachine extends Component {
    ...
    if (enable) {
      column.splice(1, 0, {
        ...
        render: (test, record) => {
          ...
          const menu = (<Menu>
            <Menu.Item>
              <span role="button" onKeyDown={null} onClick={this.textTransferAdd.bind(this, record.id)} >
                添加转换
              </span>
            </Menu.Item>

            {
              ...
              <Menu.Item>
                <span role="button" onKeyDown={null} onClick={this.textTransferDel.bind(this, record.id)} >
                  删除
                </span>
              </Menu.Item>
            }
                ...
                >
                  {stateList && stateList.length > 0 && stateList.map(s => (
                    ...
                    >
                      ...
                      <span id={s.id} name={s.name} style={{ display: 'inline-block', verticalAlign: 'middle' }}>{s.name}</span>
                    </Option>
                  ))}
                </Select>,
              )}
                  ...
                  >
                    {
                      source.statusId === 0 && (
                        <Option
                          ...
                          key="0"
                        >
                          <span style={{ display: 'inline-block' }}><FormattedMessage id="stateMachine.node.name.start" /></span>
                        </Option>
                      )
                    ...
                    }
                    {
                      source && !source.statusId && (
                        <Option
                          ...
                          key="all"
                        >
                          <span style={{ display: 'inline-block' }}><FormattedMessage id="stateMachine.node.name.all" /></span>
                        </Option>
                      )
                    }
                    {nodeData
                      ...
                      && nodeData.map(VO => VO.statusVO && (
                        ...
                        >
                          ...
                          <span id={VO.id} name={VO.statusVO.name} style={{ display: 'inline-block' }}>{VO.statusVO.name}</span>
                        </Option>
                      ))}
                  </Select>,
                  ...
                  >
                    {nodeData
                      ...
                      && nodeData.map(VO => VO.statusVO && (
                        ...
                        >
                          <div className="issue-state-machine-block" style={{ backgroundColor: VO.statusVO.type ? statusColor[VO.statusVO.type].colour : '#FFF' }} />
                          <span id={VO.statusVO.id} name={VO.statusVO.name} style={{ display: 'inline-block' }}>{VO.statusVO.name}</span>
                        </Option>
                      ))}
                  </Select>,
  ...
  addStateMachineNode = (data) => {
    ...
    stateMachineApi.addNode(stateMachineData.id, node)
      .then((nodeData) => {
  ...
  checkDelete = (cell) => {
    ...
    const that = this;
    stateMachineApi.checkBeforeDeleteNode(cell.statusId, stateMachineData.id).then((data) => {
      if (data && !data.failed) {
        if (data.canDelete) {
          confirm({
  ...
  handleConfig = (cell) => {
    ...
    history.push(`/agile/states/state-machine/${stateMachineData.id}/editconfig/${configId}?type=organization&id=${id}&name=${encodeURIComponent(name)}&organizationId=${organizationId}&orgId=${organizationId}`);
  };
  ...
  handleDeploy = () => {
    ...
    history.push(`/agile/states/state-machine/edit/${stateMachineId}/state_machine_draft?type=organization&id=${id}&name=${encodeURIComponent(name)}&organizationId=${organizationId}&orgId=${organizationId}`);
  };
  ...
  handlePublish = () => {
    ...
    stateMachineApi.publish(stateMachineData.id).then((data) => {
      this.setState({
        publishLoading: false,
      });
      if (data) {
        Choerodon.prompt(intl.formatMessage({ id: 'stateMachine.publish.success' }), 'success');
        history.push(`/agile/states/state-machine/edit/${stateMachineData.id}/state_machine_active?type=organization&id=${id}&name=${encodeURIComponent(name)}&organizationId=${organizationId}&orgId=${organizationId}`);
      } else {
        info({
  ...
  handleAllChange = (e) => {
    ...
    if (e.target.checked) {
      stateMachineApi.createAllTransferToNode(selectedCell.nodeId, stateMachineData.id)
        .then((item) => {
          transferData.push(item);
      ...
      stateMachineApi.deleteAllTransferToNode(selectedCell.allStatusTransformId)
        .then(() => {
          const cells = [];
  ...
  handleChange = async (e, code) => {
    ...
    if (name !== newName && code === 'name') {
      const res = await stateMachineApi.checkName(newName);
      if (res) {
  ...
  handleTabs = (newActiveKey) => {
    if (newActiveKey === 'text') {
      this.setState({
        headerBtnVisible: false,
      });
    }
    else {
      this.setState({
  ...
  render() {
    ...
    // 状态机流程图高度
    let graphHigh = 'calc(100vh - 315px)';
    if (!displayHeader) {
      graphHigh = 'calc(100vh - 206px)';
    } else if (status === 'state_machine_create') {
      graphHigh = 'calc(100vh - 250px)';
    } else if (status === 'state_machine_active') {
      graphHigh = 'calc(100vh - 290px)';
    }
    ...
    return (
      ...
      >
        <Header
          ...
          className="issue-state-machine-page-header"
        >
        ...
        <Breadcrumb custom>
          <Item>{name}</Item>
          <Item>
            <Link to={`/agile/states/state-machine?type=${type}&id=${id}&name=${name}&category=AGILE&organizationId=${orgId}&orgId=${orgId}`}>状态机</Link>
          </Item>
          <Item>{schemeName}</Item>
        </Breadcrumb>
        <Content className={`${prefixCls}-content`}>
            ...
            {status && status === 'state_machine_active' && !disabled && (
              <div className={`${prefixCls}-header-tip`}>
                ...
                <div className={`${prefixCls}-header-tip-action`}>
                  <Button onClick={() => this.handleDeploy()} type="primary" funcType="raised">
                    <FormattedMessage id="edit" />
                  </Button>
                </div>
              </div>
          ...
          <Tabs defaultActiveKey="graph" tabBarExtraContent={operations} onChange={this.handleTabs}>
        ...
        {this.state.deleteVisible && (
          ...
          >
            <div style={{ marginTop: 15, marginBottom: 15 }}><FormattedMessage id="stateMachine.transfer.deleteTip" /></div>
            <Select
              // label={<FormattedMessage id="stateMachine.transfer.delete.tip" />}s
