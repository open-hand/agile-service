...
class EditConfigSelect extends Component {
  ...
  handleCancel = () => {
    ...
    history.push(`/agile/states/state-machine/${machineId}/editconfig/${configId}?type=organization&id=${id}&name=${encodeURIComponent(name)}&organizationId=${organizationId}&orgId=${organizationId}`);
  }
    ...
    return (
      <Page>
        <Header
          ...
          backPath={`/agile/states/state-machine/${machineId}/editconfig/${configId}?type=${type}&id=${projectId}&name=${encodeURIComponent(name)}&organizationId=${orgId}&orgId=${orgId}`}
        >
          <Button
