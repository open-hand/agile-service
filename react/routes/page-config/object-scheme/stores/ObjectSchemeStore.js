...
class ObjectSchemeStore {
  ...
  initCurrentMenuType(data) {
    ...
    this.iamGetway = `/base/v1/${type}s/${id}`;
    this.orgId = organizationId;
  }

  // 请求分页api:    `${this.apiGetway}/object_scheme?page=${page}&size=${size}&organizationId=${this.orgId}`, filter,
  loadObjectScheme = filter => axios.post(
    `${this.apiGetway}/object_scheme?organizationId=${this.orgId}`, filter,
  ).then((data) => {
    if (data && !data.failed) {
  ...
  loadLookupValue = code => axios.get(`/agile/v1/lookup_values/${code}`);
