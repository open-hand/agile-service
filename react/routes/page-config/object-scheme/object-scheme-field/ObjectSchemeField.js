...
import {
  ...
  DatePicker, InputNumber, Checkbox, Modal,
} from 'choerodon-ui';
...
function ObjectSchemeField(props) {
  ...
  const { objectSchemeStore, intl, AppState } = context;
  const { form } = props;
  // use State
  ...
  const loadFieldById = () => {
    ...
    // console.log('进入filed 加载', fieldId);
    objectSchemeStore.loadFieldDetail(fieldId).then((data) => {
      if (data) {
  ...
  const saveEdit = () => {
    ...
    form.validateFieldsAndScroll((err, data) => {
      if (!err) {
        ...
        setSubmitting(true);
        objectSchemeStore.updateField(fieldId, postData)
          .then(() => {
            // this.setState({
            //   submitting: false,
            // });
  ...
  function render() {
    ...
    return (
      ...
      >
        <Form layout="vertical" onSubmit={null} className="c7nagile-form">
          <FormItem>
