...
class PageStore {
  ...
  // 分页请求api：`${this.apiGetway}/page?page=${page}&size=${size}&organizationId=${this.orgId}`, filter,
  loadPage = filter => axios.post(
    `${this.apiGetway}/page?organizationId=${this.orgId}`, filter,
  ).then((data) => {
    if (data && !data.failed) {
