...
export type IGanttLegendKey = 'todo' | 'doing' | 'complete' | 'estimatedStartEnd' | 'actualStartEnd' |
'delay' | 'progress' | 'sprint' | 'holidays'
...
function GanttLegend({ hiddenLegendKeys }: { hiddenLegendKeys: string[] }) {
  ...
  return (
    ...
    >
      {defaultLegends.filter((legend) => !hiddenLegendKeys.includes(legend.key)).map((legend) => legend.element)}
    </div>
  );
}
