                    ...
                    <td style={{ width: '94px' }}>
                      <a
                        ...
                        onClick={() => {
                          ...
                          history.push(
                            `/agile/work-list/issue?type=${type}&id=${id}&name=${encodeURIComponent(urlParams.name)}&organizationId=${organizationId}&orgId=${organizationId}&paramType=typeCode&paramId=${item.typeName}&paramName=${encodeURIComponent(`${item.name || '未分配'}下的问题`)}&paramUrl=reportBoard`,
                          );
                        }}
                      >
                        {item.value}
