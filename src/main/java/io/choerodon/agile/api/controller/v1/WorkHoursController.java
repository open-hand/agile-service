...
public class WorkHoursController {
    ...
    public ResponseEntity<Page<IssueListFieldKVVO>> pageQueryIssues(@ApiParam(value = "项目id", required = true)
                                                                    @PathVariable(name = "project_id") Long projectId,
                                                                    @RequestParam(name = "organizationId") Long organizationId,
        ...
        return Optional.ofNullable(workHoursService.pageQueryIssues(organizationId, Arrays.asList(projectId), pageRequest, containsSubIssue, searchVO))
                .map(result -> new ResponseEntity<>(result, HttpStatus.OK))
    ...
    public ResponseEntity<Page<IssueWorkHoursVO>> pageQueryAssignee(@ApiParam(value = "项目id", required = true)
                                                                    @PathVariable(name = "project_id") Long projectId,
                                                                    @RequestParam(name = "organizationId") Long organizationId,
                                                                    PageRequest pageRequest,
                                                                    @RequestBody SearchVO searchVO) {
        EncryptionUtils.decryptSearchVO(searchVO);
        return Optional.ofNullable(workHoursService.pageQueryAssignee(organizationId, Arrays.asList(projectId), pageRequest, searchVO))
                .map(result -> new ResponseEntity<>(result, HttpStatus.OK))
                .orElseThrow(() -> new CommonException("error.issue.work.hours.query"));
    }
