...
To install the chart with the release name `agile-service`:

```console
$ helm repo add c7n https://openchart.choerodon.com.cn/choerodon/c7n
$ helm repo update
$ helm install agile-service c7n/agile-service
```
...
| containerPort.actuatorPort | int | `8379` | server management port |
| containerPort.serverPort | int | `8378` | server port |
...
| extraEnv.CHOERODON_CLEANPERMISSION | bool | `false` |  |
| extraEnv.EUREKA_CLIENT_SERVICEURL_DEFAULTZONE | string | `"http://register-server:8000/eureka/"` | Eureka client service url |
| extraEnv.SERVICES_ATTACHMENT_URL | string | `"http://minio.example.com/agile-service"` |  |
| extraEnv.SERVICES_CHAIN_MAX_DEPTH | int | `20` |  |
| extraEnv.SERVICES_DOMAIN_URL | string | `"http://example.com"` |  |
...
| extraEnv.SPRING_DATASOURCE_URL | string | `"jdbc:mysql://localhost:3306/agile_service?useUnicode=true&characterEncoding=utf-8&useSSL=false&useInformationSchema=true&remarks=true&serverTimezone=Asia/Shanghai"` | Datasource url |
| extraEnv.SPRING_DATASOURCE_USERNAME | string | `"choerodon"` | Datasource username |
| extraEnv.SPRING_REDIS_DATABASE | int | `12` | Redis database |
...
| image.repository | string | `"c7n/agile-service-business"` | service image repository |
...
| service.nodePort | object | `{"actuator":30143,"server":30142}` | Specify the nodePort value for the LoadBalancer and NodePort service types. |
| service.port | object | `{"actuator":8379,"server":8378}` | server Service port |
